...
import {moduleName as roleManagerModuleName, RoleManagerModuleExport} from 'modules/roleManager'
...
export default defineScreepModule<
  {
    [roleManagerModuleName]: RoleManagerModuleExport;
...
>({
  name: moduleName,
  inject: [roleManagerModuleName, sourceModuleName],
  binding() {
    const {onCheckAndCreate, onRoleDead: onCreepDead} = this.modules[roleManagerModuleName]

    onCheckAndCreate<>((spawn) => {
